(function webpackUniversalModuleDefinition(root, factory) {
  ...
  else if (typeof exports === "object") exports["PaletteGenerator"] = factory();
  else root["PaletteGenerator"] = factory();
      ...
      /***/ function (module, exports, __webpack_require__) {
        ...
        function checkRange(value, maxValue, label) {
          if (isNaN(value) || 0 > value || value > maxValue)
            throw new RangeError(
              value + " for " + label + " is not between 0 and " + maxValue
            );
        }
        ...
        function rgb2hex(rgbColor) {
          return (
            ...
            (1 > rgbColor.alpha
              ? decimal2hex(Math.round(255 * rgbColor.alpha))
              : "")
          );
        }
        ...
        function hsl2rgb(hslColor) {
          var b =
            (1 - Math.abs(2 * hslColor.lightness - 1)) * hslColor.saturation;
          return kb(
            hslColor.hue,
            hslColor.alpha,
            b,
            Math.max(0, hslColor.lightness - b / 2)
          );
        }
        ...
        function hex2decimal(hexColor) {
          if (!/^[a-fA-F0-9]+$/.test(hexColor))
            throw Error("Invalid hex string: " + hexColor);
        ...
        HSLColor.prototype.rotate = function (hueAdjustment) {
          return new HSLColor(
            (this.hue + hueAdjustment + 360) % 360,
            this.saturation,
            this.lightness,
            this.alpha
          );
        ...
        function hsb2hsl(hsbColor) {
          var b = minMax(
              ((2 - hsbColor.saturation) * hsbColor.value) / 2,
              0,
              1
            ),
            c = 0;
          0 < b &&
            1 > b &&
            (c =
              (hsbColor.saturation * hsbColor.value) /
              (0.5 > b ? 2 * b : 2 - 2 * b));
          c = minMax(c, 0, 1);
        ...
        var REDUCED_CHROMA_TOLERANCE = [
          1.762442714, 4.213532634, 7.395827458, 8.07174158, 9.89634504,
          11.37591477, 12.27071136, 13.54160806, 14.35916727, 16.88410864,
        ];
        ...
        function findClosestGoldenPalette(labColor) {
          ...
          for (
            var c = Infinity, d = goldenPalettes[0], e = -1, paletteIndex = 0;
            paletteIndex < goldenPalettes.length;
            paletteIndex++
          ) {
            for (
              var colorIndex = 0;
              colorIndex < goldenPalettes[paletteIndex].length && 0 < c;
              colorIndex++
            ) {
              ...
              t =
                ...
                (1 -
                  Math.sqrt(
                    Math.pow(t, 7) / (Math.pow(t, 7) + Math.pow(25, 7))
                  ));
              ...
              k = Math.sqrt(
                Math.pow(
                  ...
                  (t / (1 * q)) *
                    ...
                    Math.sin(
                      (60 * Math.exp(-Math.pow((m - 275) / 25, 2)) * Math.PI) /
                        180
                    ) *
                    -2 *
                    (N / (1 * H))
              ...
              k < c &&
                ((c = k), (d = goldenPalettes[paletteIndex]), (e = colorIndex));
            }
          }
        ...
        function generatePalette(sourceRgbColor) {
          var goldenPalettes =
            arguments.length > 1 && arguments[1] !== undefined
              ? arguments[1]
              : GOLDEN_ACCENT_PALETTES;
          var lightnessTolerance =
          ...
          var goldenPalette = findClosestGoldenPalette(
            sourceLabColor,
            goldenPalettes
          );
          var goldenColors = goldenPalette.colors;
          var closestGoldenLabColor =
            goldenColors[goldenPalette.closestReference];
          ...
          return goldenColors.map(function (goldenLabColor, index) {
            ...
            var chroma = isGoldenColorGreyInMiddle
            ...
            lightnessMaximum = Math.max(
              lchColor.lightness - lightnessMinimumStep,
              0
            );
